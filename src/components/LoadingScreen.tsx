...
import { loadingTerminalCode } from "@/data/personalData";
...
const codeLines = [
  ...
  "portfolio.compileWorkSamples();",
  "portfolio.optimizePerformance();",
  "portfolio.initializeCreativityHub();",
...
export const LoadingScreen = ({ onComplete }: { onComplete: () => void }) => {
  ...
  const connections = [
    ...
    { x1: 20, y1: 50, x2: 50, y2: 45 },
    { x1: 20, y1: 50, x2: 50, y2: 65 },
    { x1: 20, y1: 70, x2: 50, y2: 65 },
    { x1: 20, y1: 70, x2: 50, y2: 85 },
    // Hidden to output
    { x1: 50, y1: 25, x2: 80, y2: 40 },
    { x1: 50, y1: 45, x2: 80, y2: 40 },
    { x1: 50, y1: 65, x2: 80, y2: 60 },
    { x1: 50, y1: 85, x2: 80, y2: 60 },
  ];
    ...
    if (currentLine < codeLines.length) {
      ...
      const typeInterval = setInterval(() => {
        if (charIndex <= line.length) {
          setDisplayedCode(
            codeLines.slice(0, currentLine).join("\n") +
              "\n" +
            ...
            <div className="flex justify-between text-sm text-muted-foreground mb-2">
              {(() => {
                const stepIndex = Math.floor(
                  (progress / 100) * loadingTerminalCode.length
                );
                const stepText =
                  loadingTerminalCode[
                    Math.min(stepIndex, loadingTerminalCode.length - 1)
                  ];
